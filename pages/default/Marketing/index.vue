        ...
        <div class="swiper_pic_section">
          <banner-swiper>
            ...
            >
              <nuxt-link
                class="swiper-img"
                :to="`/marketing/series?id=${item.alias}`"
              >
        ...
        <div class="material_list_section">
          <div class="left_wrap">
            ...
            >
              ...
              >
                ...
                <div v-if="promotionName != item.name" class="third_wrap">
                  <div
          ...
          <div class="right_wrap">
            <div class="material_list_content">
              ...
              <template v-if="materialList.list.length>0">
                <material-list
...
export default {
  ...
  async fetch () {
    /** console.log('如果没有登录状态就踢回登录页') */
    if (!this.$store.getters['login/checkLoginStatus']) {
      this.$router.push('/login')
    } else {
      const query = JSON.parse(JSON.stringify(this.$route.query))
      // console.log(query, 'mkt首页路由参数')

      /** 初始获取物料列表数据 */
      this.getMaterialList({ local: true, query })
  ...
  watch: {
    ...
    thirdId () {
      ...
      this.useRouteParam({ router: this.$router, url: '/marketing' })
    },
    firstIndex () {
      this.handlePagination(this.page)
      this.useRouteParam({ router: this.$router, url: '/marketing' })
  ...
  methods: {
    ...
    brandNameEvent (index, item) {
      ...
      // console.log(item, '一级栏目切换 得到二级三级栏目')
    },

    /** 设置三级的显示状态 */
    selectedThirdMenuEvent (index, subId, thirdId, name) {
      // console.log('设置三级的显示状态', { index, subId, thirdId, name }) // index：三级分类索引, subId:二级分类id（品牌分类，产品物料等）,subAndThirdList:二级列表, thirdId, name：当前品牌名字
