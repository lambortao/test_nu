  ...
  <div>
    <client-only>
      <div class="container site-is-default ">
            ...
            <div>
              ...
              >
                <span v-for="item in anchorGroup[selectedIndex].anchor" :key="item.top" :style="{ top: item.top, left: item.left }">
                  <span
                    :class="parseInt(item.left) < 50 ? 'right' : 'left'"
                    :style="{ width: `${getTextWidth(item.option.title_en + item.option.title)}px` }"
                  >{{ item.option.title_en }}{{ item.option.title }}</span>
                </span>
              </div>
            </div>
          </div>
        ...
        <div class="main">
          <div class="title">
            <h5>{{ caseDetail.subtitle }}      {{ caseDetail.title }}</h5>
          </div>
          <div class="area">
            <div class="country">
              {{ caseDetail.city }}
          ...
          <div class="artlice">
            <p v-if="caseDetail.content" v-html="caseDetail.content" />
          </div>
          <div
...
export default {
  ...
  head () {
    return { title: `Nine United官网-案例-${this.caseDetail.title}` }
  },
  computed: {
    ...mapState('case', {
      /** 详情页数据 */
