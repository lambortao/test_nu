    ...
    <div class="container default-width site-is-default">
      <div class="case_box">
        ...
        <div class="case-content">
          <header class="case-content-header">
            <div class="header-case-list">
              <div
                ...
                @click="caseTracking({event:'filter_inspiration',mode:'Inspiration',label:item.name,filterLabel:'default'})"
              >
                <nuxt-link
            ...
            <div class="totalcase">
              <div class="brand-lists">
                <nuxt-link
                  ...
                  :to="`/case?id=${selectedCase}&brand=${encodeURIComponent(item.name)}`"
                  class="brand-tab"
                >
                  <div @click="brandSelect(item.name)">
                    {{ item.name }}
                  </div>
                </nuxt-link>
              </div>
              <!-- <div class="totals">
                {{ caseLists.total? caseLists.total:0 }} 案例
              </div> -->
            </div>
          </header>
          ...
          <div v-if="!caseLists.total && !$fetchState.pending" class="empty-area">
            <div class="empty">
...
import MobileCase from '~/pages/mobile/case/index.vue'
...
export default {
  ...
  methods: {
    goBrand (name) {
      this.$router.push({ path: '/case', query: { id: this.selectedCase, brand: name } }, () => {}, () => {})
    },
    getMore () {
