<template>
  <div
    ref="selectRef"
    :class="['select-singin', className]"
  >
    <span class="label">{{ label }}</span>
    <div class="select-drow">
      <span
        v-if="defaultselect"
        :class="{colorGray:defaultselect}"
      ...
      >{{ value || '全部' }}</span>
      <div
        v-if="dropVisible&&!defaultselect"
...
export default {
  ...
  methods: {
    handleOptions (key) {
      ...
      const filterLabel = this.label === '城市' ? 'City' : this.label === '国家' ? 'Country' : this.label
      this.$emit('tracking', { event: 'filter_store', mode: 'Store', label: key, filterLabel })
    }
  }
}
</script>
