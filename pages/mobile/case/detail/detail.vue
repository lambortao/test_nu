<template>
  <div class="container">
    <div class="showimg">
      <div class="main">
        <div class="button">
          <div v-for="(items, ind) in group" :key="ind" class="icons" :class="indexz === ind ? 'active': '' " @click="getUrl(items.img,ind)">
...
export default {
  ...
  methods: {
    ...
    async getDetailList () {
      ...
      this.group.splice(this.indexz, this.group.length - this.indexz)
    ...
    async getUrl (url, indexz) {
      ...
      this.anchorImgWidth = await this.getImgWidth(this.url)
      this.indexz = indexz
      this.anchor = this.group[indexz].anchor
    },
    showDescribe (item) {
