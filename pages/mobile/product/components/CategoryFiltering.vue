  ...
  <div :class="['category-filtering', { tall: isTall }, { active: visible }]" @click="closeMenu">
    <div class="content" @click.stop="eventMenu">
      <header class="pt24">
        ...
        <nuxt-link to="/product" tag="p" @click="closeMenu">查看全部</nuxt-link>
      </header>
      ...
      <footer>
        <button @click="setSelectedProductList({ mode: 'reset' })">重置</button>
        <button @click="confirmRouteParam">确定</button>
      </footer>
    </div>
  </div>
</template>
