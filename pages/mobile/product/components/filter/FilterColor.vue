...
<template>
  <div class="filter">
    <p class="title" @click.stop="toggleFilterKeys">颜色</p>
    <section v-if="showFilterKey === 'color'" class="filter-content">
      <div class="content">
        <h3>颜色选择</h3>
        <div v-if="colorShow&&colorList.length>0&&productCategorySelect.length>0 " class="tags-list">
          <div v-for="(item,idx) in colorList" :key="idx" class="each" @click="select(item.name)">
            <div class="eachBox">
              <div :class="['circle',value.indexOf(item.name) > -1 ? 'borderActive':'']">
                <div class="circular">
                  <img :src="item.image" alt="">
                </div>
              </div>
              <p :class="['font',value.indexOf(item.name) > -1 ? 'fontActive':'']">{{ item.name }}</p>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    // 下拉框
    toggleFilterKeys () {
      this.$store.commit('productBrand/setShowFilterKey', 'color')
    },
    // 筛选颜色
    select (item) {
