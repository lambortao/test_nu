...
<template>
  <div class="filter">
    <p class="title" @click.stop="toggleFilterKeys">材质</p>
    <section v-if="showFilterKey === 'texture'" class="filter-content">
      <div class="content">
        ...
        <div v-if="textureShow&&textureList.length>0&&productCategorySelect.length>0 " class="tags-list">
          <div v-for="(item,idx) in textureList" :key="idx" class="each" @click="select(item.name)">
            <div class="eachBox">
        ...
        <div v-else class="empty">
          {{ productCategorySelect.length > 0 ? '暂无可筛选材质' : '请选择品类后再进行材质筛选' }}
        </div>
      </div>
      <footer>
