...
<template>
  <div class="filter">
    <p class="title" @click.stop="toggleFilterKeys">品牌</p>
    <section v-if="showFilterKey === 'brand'" class="filter-content">
      <h3>品牌选择</h3>
      <div class="content">
        <div class="tags-list">
          <p :class="['brand_item', { active: brandAllCheck }]" @click="selectAllEvent()">全部</p>
          <p v-for="item in brandList" :key="item.alias" :class="['brand_item', { active: value.includes(item.alias) }]" @click="brandSelect({item,isSelect:value.includes(item.alias)})">{{ item.name }}</p>
        </div>
      </div>
      <footer>
