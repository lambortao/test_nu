...
export default {
  ...
  computed: {
    ...
    newBirthday: {
      set (value) { this.$store.commit('users/setNewData', { key: 'birthday', value }) },
      get () { return this.$store.getters['users/getNewData'].birthday }
    }
  },
  methods: {
    async confirm () {
      try {
        ...
        this.$store.commit('users/setNewData', { key: 'birthday', value: '' })
      } catch (error) {
        console.log(error)
      }
    }
