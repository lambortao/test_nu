  ...
  methods: {
    claerDataEvent () {
      if (this.directorySubId) {
        ...
        this.$router.push(`/en/marketing?categories=${this.directorySubId}&tag=${this.selectedThirdId}&brand=${name}`)
      } else {
        this.$router.push(`/en/marketing?brand=${this.brandName}`)
      }
    },
    searchEvent () {
      // 判断input输入内容全是空格
      ...
      if (value !== 0) {
        const name = encodeURIComponent(this.brandName)
        this.$router.push(`/en/marketing/search?search=${this.inputValue}&brand=${name}`)
