    ...
    <template v-if="materialList && materialList.list.length">
      <div class="material_list">
        <nuxt-link
          ...
          pagedifferent
          :to="`/en/marketing/detail?id=${item.alias}`"
        >
          <div class="market_content" @click="macketTracking({label:item.title,alias:item.alias})">
            <div
              class="material_img_wrap"
              :style="{ backgroundImage: `url(${item.list_tmb})` }"
            >
              <div v-if="pagedifferent==='home'" class="material_new">
                <div class="file_txt">
                  {{ item.is_file ? "File" : "Video" }}
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    macketTracking ({ label, alias }) {
      const paget = this.pagedifferent
      console.log(paget)
