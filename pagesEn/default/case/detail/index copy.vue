    ...
    <client-only>
      <div class="container site-is-default ">
            ...
            <div class="maina">
              <swiper v-if="group" ref="swiperTop" class="swiper swiper-b" :options="swiperOption2">
                <swiper-slide v-for="(items, ind) in group" :key="ind">
                  <div :style="`background: url(${items.img}) no-repeat`" class="img" :class="indexz === ind ? 'active': '' " @click="getUrl(items.img,ind)" />
                </swiper-slide>
              </swiper>
        ...
        <div class="main">
          ...
          <div class="title">
            <h5>{{ detailList.subtitle_en }}      {{ detailList.title_en }}</h5>
          </div>
          <div class="area">
            <div class="country">
              {{ detailList.city_en }}
          ...
          <div class="artlice">
            <p v-if="detailList.content" v-html="detailList.content_en" />
          </div>
...
export default {
  ...
  async fetch () {
    this.postData = this.$route.query.brandId
    await this.$store.dispatch('caseDetail/useDetail', this.postData)
    ...
    this.anchor = this.group[this.indexz].anchor
  },
  // head: {
  //   title: 'Nine United官网-'
  // },
  head () {
    return { title: 'Nine United-' + this.titles }
  },
  computed: {
    Detail () {
  ...
  methods: {
    ...
    async getUrl (url, indexz) {
      console.log(indexz, 'indddddd')
      this.url = url
      this.anchorImgWidth = await this.getImgWidth(this.url)
      this.$nextTick(() => {
        this.personScroll()
      ...
      this.indexz = indexz
      this.anchor = await this.group[indexz].anchor
    ...
    async prev () {
      ...
      this.url = this.group[this.indexz].img
      this.anchorImgWidth = await this.getImgWidth(this.url)
      this.anchor = await this.group[this.indexz].anchor
      this.product = []
      this.title = ''
      this.$nextTick(() => {
        this.personScroll()
