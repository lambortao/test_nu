<template>
  <div class="select-singin">
    ...
    <div class="select-drow">
      <span @click.stop="toogleVisible">{{ itemName }}</span>
...
export default {
  ...
  computed: {
    itemName () {
      return (this.value.length > 0 && this.value.length < this.list.length) ? this.value.join(',') : 'ALL'
    }
  },
  methods: {
    toogleVisible () {
    ...
    selectedAll () {
      ...
      if (!data.length) {
        ...
        this.$emit('tracking', { event: 'filter_store', moede: 'Store', label: 'ALL', filterLabel: 'Brands' })
