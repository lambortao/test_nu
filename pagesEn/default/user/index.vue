<template>
  <div>
    <div class="site-is-default default-width">
      <div class="user_box default-width">
        <div class="user_brief_box">
          <div class="user_brief_left">
            ...
            <div class="user_tag">
              <p>Hello {{ memberData.fullname || memberData.nickname }}</p>
              <identity-tags />
            </div>
          </div>
          <div class="user_brief_right">
            <p>
              Want to change your account? <span @click="logout">Log out</span>
            </p>
          </div>
        </div>
        <div class="user-box-main">
          <div class="user-box-main-tab">
            <div
              :class="['favorite', { active: showKey === 'favorite' }]"
              @click="setKey('favorite')"
            >
              <span />
              <p>Favorites</p>
            </div>
            <!--营销工具包  可见：经销商、员工; 普通会员不可见 -->

            <nuxt-link
              v-if="getIdentity.includes('3') || getIdentity.includes('6')"
              :to="`/en/marketing?categories=${directorySubId}&tag=${directoryThirdId}&brand=${directoryBrandName}`"
            >
              <div class="marketing">
                <span />
                <p>Marketing Hub</p>
            ...
            <div
              v-if="false"
              :class="['help', { active: showKey === 'help' }]"
              @click="setKey('help')"
            >
              <span />
            ...
            <div
              :class="[
                'setting',
                { active: ['setting', 'privacy'].includes(showKey) },
              ]"
              @click="setKey('setting')"
            >
              <span />
              <p>Setting</p>
            </div>
      ...
      <success-dialog v-if="editStatus" done_text="Modified successfully" />
    </div>
...
import MobileUser from '~/pagesEn/mobile/user'
import SuccessDialog from '~/componentsEn/common/Dialog/success.vue'
import identityTags from '~/componentsEn/IdentityTags/index.vue'
...
export default {
  ...
  created () {
    ...
    if (!this.$store.getters['login/checkLoginStatus']) {
      ...
      this.getIdentity.includes('3') ||
      this.getIdentity.includes('6')
    ) {
      this.$nextTick(async () => {
