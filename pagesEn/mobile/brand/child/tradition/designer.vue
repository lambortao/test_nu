...
export default {
  ...
  methods: {
    async getList () {
      const list = await this.$axios.$post('/getBrandDesignerList', { brand: 'TRADITION' })
      this.designerList = list
      this.count = list
    },
    goStylist (id) {
      this.$router.push({ path: '/en/stylist', query: { designerId: id, brand: 'TRADITION', count: this.count } }, () => {}, () => {})
    }
  }
}
</script>
