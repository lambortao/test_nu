  ...
  <client-only>
    <div class="mobile-login wh-full">
      ...
      <div class="login-box c-whire">
        <div v-if="false" class="login-header-en">
        ...
        <form-label v-else>
          <country-phone
            v-model="loginId"
            :country-tag="$store.state.login.loginEn.area_code"
            @changeCountryTag="value => $store.commit('login/setLoginEnData', { key: 'area_code', value })"
          />
        </form-label>
        <form-label>
          <en-minput v-model="password" autocomplete="off" type="password" placeholder="Password" />
        </form-label>
        <div class="forget-password" @click="changeFlow('forget')">Forget Password?</div>
        <change-flow-vue v-model="isMail" />
      </div>
      <div class="confirm-btn bottom-btn">
        <mobile-aggreement />
        <big-button :disabled="!isActive || submit" :style="{ marginBottom: '10px' }" @click="handleLogin">{{ btnText }}</big-button>
        <big-button :style="{ backgroundColor: '#fff', color: '#000', border: '1px solid #000' }" @click="changeFlow('register')">Register</big-button>
      </div>
    </div>
  </client-only>
</template>
